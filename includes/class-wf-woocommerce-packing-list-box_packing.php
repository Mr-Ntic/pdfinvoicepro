    ...
    class Wf_Woocommerce_Packing_List_Box_packing
    {
        ...
        public function create_order_package($order, $template_type) 
        {
            ...
            if(!empty($out)){
                ...
                {
                    $count=count($out);
                    $package_count_text = $item_count.__(' of ', 'wt_woocommerce_invoice_addon').$count;
                    $value[0]['count']= apply_filters('wt_pklist_alter_package_count_text',$package_count_text,$item_count,$count);
                    $items[$key]=$value;
                    $item_count++;
                }
                $out=$items;
        ...
        public function wf_pklist_create_order_single_package($order) {
            ...
            $show_package_after_refund = apply_filters('wt_pklist_show_package_after_refund',true,$order,$template_type);
            ...
            foreach ($order_items as $id => $item) 
            {
                ...
                if($product) 
                {
                    ...
                    if($show_package_after_refund){
                        $item_qty_refunded = $order->get_qty_refunded_for_item( $id );
                        $item_quantity = (int)$item['qty'] - abs($item_qty_refunded);
                        if(abs($item_qty_refunded) === (int)$item['qty']){
                            continue;
                        }
                    }
        ...
        public function wf_packinglist_get_table_content($order, $order_package, $show_price = false)
        {
            ...
            foreach ($order_package as $order_package_individual_item) {
                $weight_of_item += (!empty($order_package_individual_item['weight'])) ? $order_package_individual_item['weight'] * $order_package_individual_item['quantity'] : __('0', 'wt_woocommerce_invoice_addon');
            }

            if (key_exists('Value', $order_package)) {
                $weight = ($order_package['Value'] != '') ? $order_package['Value'] : __('n/a', 'wt_woocommerce_invoice_addon');
            } else {
            ...
            $orderdetails = array(
                ...
                'weight' => ($weight != '') ? $weight . ' ' . get_option('woocommerce_weight_unit') : __('n/a', 'wt_woocommerce_invoice_addon'),
        ...
        {   
            ...
            { 
                ...
                $newbox = new WF_Boxpack_Box($box['name'],$box['length'], $box['width'], $box['height'], $box['box_weight']);
                $boxpack->add_box($newbox);
                if (isset($box['id']) && $newbox) {
            ...
            foreach ($orderItems as $id => $orderItem) {
                if (!empty($orderItem)) {
                    ...
                    $item_quantity = $orderItem['qty'];
                    if($show_package_after_refund){
                        $item_qty_refunded = $order->get_qty_refunded_for_item( $id );
                        $item_quantity = (int)$orderItem['qty'] - abs($item_qty_refunded);
                    }
                    $items[] = array('data' => $product_data, 'quantity' => $item_quantity, 'order_item_id' => $id);
                }
            }
            if (!empty($items)) {
        ...
        public function wf_pklist_create_order_indvidual_item_package($order) {
            ...
            foreach ($order_items as $id => $item) {
                ...
                if($product) 
                {
                    ...
                    for ($item_count = 0; $item_count < $item_quantity; $item_count++) {
                        $packinglist_package[][] = array(
                            'sku' => $product->get_sku(),
                            'name' => $product->get_name(),
                            'type' => $product_product_type,
...
if(!class_exists('WF_Boxpack')){
    class WF_Boxpack {
         ...
         * @return void
         */
        public function add_item($box_name,$length, $width, $height, $weight, $value = '', $meta = array(),$order_item_id='') {
            $this->items[] = new WF_Boxpack_Item($box_name,$length, $width, $height, $weight, $value, $meta,$order_item_id);
        }
         ...
         * @return void
         */
        public function add_box_old($box_name,$length, $width, $height, $weight = 0) {
    ...
    WF_Boxpack::get_instance();
}

/**
 * WF_Boxpack_Box class.
 */
if(!class_exists('WF_Boxpack_Box')){
    class WF_Boxpack_Box {
        ...
        public function __construct($box_name,$length, $width, $height, $weight = 0){
            if(null !== $box_name && null !== $length && null !== $width && null !== $height){
         ...
         * Get Instance
         */
        public static function get_instance($box_name,$length, $width, $height, $weight = 0)
        {
            if(self::$instance==null)
            {
                self::$instance = new WF_Boxpack_Box($box_name,$length, $width, $height, $weight);
            }
            return self::$instance;
        }
    ...
    WF_Boxpack_Box::get_instance(null,null,null,null);
}

/**
 * WF_Boxpack_Item class.
 */
if(!class_exists('WF_Boxpack_Item')){
    class WF_Boxpack_Item {
         ...
         * @return void
         */
        public function __construct($box_name,$length, $width, $height, $weight, $value = '', $meta = array(),$order_item_id='') {
            $dimensions = array($length, $width, $height);
