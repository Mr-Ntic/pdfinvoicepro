		...
		var wt_pklist_custom_field_editor=
		{
			...
			reg_delete:function()
			{
				$(document).on('click','.wt_pklist_custom_field_delete',function(){
					...
					if(confirm(wf_pklist_params_ipc_common_param.msgs.sure))
					{
			...
			{
				var ajx_data='action=wt_pklist_custom_field_list_view&_wpnonce='+wf_pklist_params_ipc_common_param.nonces.wf_packlist+'&wt_pklist_custom_field_type='+jQuery('[name="wt_pklist_custom_field_btn"]').data('field-type');
				...
				$('.wf_popover-content').find('.wt_pklist_custom_field_tab_content[data-id="list_view"]').html(wf_pklist_params_ipc_common_param.msgs.please_wait);
				jQuery.ajax({
					url:wf_pklist_params_ipc_common_param.ajaxurl,
					data:ajx_data,
	            	type: 'POST',
					...
					error:function()
					{
						$('.wt_pklist_custom_field_tab_content[data-id="list_view"]').html(wf_pklist_params_ipc_common_param.msgs.error);
					}
				});
			}
		}
...
var wf_popover={
	Set:function()
	{
		...
		jQuery('[data-wf_popover="1"]').on('click',function(){
			...
			{
				var template='<div class="wf_popover"><h3 class="wf_popover-title"></h3><span class="wt_popover_close_top popover_close" title="'+wf_pklist_params_ipc_common_param.msgs.close+'">X</span>'
				+'<form class="wf_popover-content"></form><div class="wf_popover-footer">'
				+'<button name="wt_pklist_custom_field_btn" type="button" id="wt_pklist_custom_field_btn" class="button button-primary">'+wf_pklist_params_ipc_common_param.msgs.save+'</button>'
				+'<button name="popover_close" type="button" class="button button-secondary popover_close">'+wf_pklist_params_ipc_common_param.msgs.close+'</button>'
	...
	{	    		
		jQuery('[name="wt_pklist_custom_field_btn"]').on('click',function(){
	        ...
	        if(empty_fields>0){
	        	alert(wf_pklist_params_ipc_common_param.msgs.enter_mandatory_fields);
	        ...
	        if(0 === empty_fields){
    			...
    			if ("" !== custom_meta_key && (jQuery.isNumeric(custom_meta_key) || custom_meta_key.match(/^\d+$/))) {
    				alert(wf_pklist_params_ipc_common_param.msgs.enter_mandatory_non_numeric_fields);
    				return false;
    			}
	        }
	        var elm=jQuery(this);
	        ...
	        data+='&action=wf_pklist_advanced_fields&_wpnonce='+wf_pklist_params_ipc_common_param.nonces.wf_packlist+'&wt_pklist_custom_field_btn&wt_pklist_custom_field_type='+elm.data('field-type');	
	        jQuery.ajax({
				url:wf_pklist_params_ipc_common_param.ajaxurl,
				data:data,
				dataType:'json',
